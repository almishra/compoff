...
int validate(VARIANT var, int device)
{
  for(int a=0; a<LA; a++)
    for(int b=0; b<LB; b++)
      for(int c=0; c<LC; c++)
    ...
    for(int b=0; b<LB; b++) {
      for(int c=0; c<LC; c++) {
        for(int d=0; d<LD; d++) {
          ...
          int xx = 0;
          for(int i=0; i<LB; i++) xx=1*1+4/4-2;
          for(int count = 0; count <COUNT; count++) {
          for(int n=0; n<N; n++) {
            vectorC[a][b][c][d][n] = xx;
          }
          for(int e=0; e<LE; e++) {
            float temp[N];
            for(int n=0; n<N; n++) {
...
//  for(int device = 0; device < num_dev; device++) {
    ...
    switch(var) {
      case STATIC1:
        matrix_vector_collapse1_static(matA, vecB, vecC, 0);
        break;
      case STATIC2:
        matrix_vector_collapse2_static(matA, vecB, vecC, 0);
        break;
      case STATIC3:
        matrix_vector_collapse3_static(matA, vecB, vecC, 0);
        break;
      case STATIC4:
        matrix_vector_collapse4_static(matA, vecB, vecC, 0);
        break;
        break;
      case DYNAMIC1:
        matrix_vector_collapse1_dynamic(matA, vecB, vecC, 0);
        break;
      case DYNAMIC2:
        matrix_vector_collapse2_dynamic(matA, vecB, vecC, 0);
        break;
      case DYNAMIC3:
        matrix_vector_collapse3_dynamic(matA, vecB, vecC, 0);
        break;
      case DYNAMIC4:
        matrix_vector_collapse4_dynamic(matA, vecB, vecC, 0);
        break;
        break;
      case GUIDED1:
        matrix_vector_collapse1_guided(matA, vecB, vecC, 0);
        break;
      case GUIDED2:
        matrix_vector_collapse2_guided(matA, vecB, vecC, 0);
        break;
      case GUIDED3:
        matrix_vector_collapse3_guided(matA, vecB, vecC, 0);
        break;
      case GUIDED4:
        matrix_vector_collapse4_guided(matA, vecB, vecC, 0);
        break;
        break;
      default:
        break;
...
//    print(vecC[0]);
    print();
#ifdef VALIDATE
//    if(validate(var, vecC[0], 0) == 0)
    if(validate(var, vecC, 0) == 0)
...
#pragma omp target map(matA[0:LA][0:LB][0:LC][0:LD][0:LE][0:M][0:N]) \
  ...
  vecC = (float (*)[LB][LC][LD][N]) malloc(sizeof(float)*LA*LB*LC*LD*N);
...
#pragma omp target enter data \
  map(alloc: vecC[0:LA][0:LB][0:LC][0:LD][0:N]) \
  map(to: matA[0:LA][0:LB][0:LC][0:LD][0:LE][0:M][0:N]) \
  map(to: vecB[0:LA][0:LB][0:LC][0:LD][0:LE][0:N])
  gettimeofday(&tv2, NULL);
  long long runtime = (tv2.tv_sec - tv1.tv_sec) * 1000000;
  runtime += tv2.tv_usec - tv1.tv_usec;
  fprintf(fp, "DATA,,0,%ld,0,%d,%d,%d,%d,%d,%d,%d,%d,%lld,%0.4f\n", device, VAR_STR[var].c_str(), mem_to, COUNT, LA, LB, LC, LD, LE, M, N, runtime, (double)(runtime/1000000.0));
  fflush(fp);
  printf("DATA,,0,%ld,0,%d,%d,%d,%d,%d,%d,%d,%d,%lld,%0.4f\n", device, VAR_STR[var].c_str(), mem_to, COUNT, LA, LB, LC, LD, LE, M, N, runtime, (double)(runtime/1000000.0));
...
#pragma omp target exit data \
  map(from: vecC[0:LA][0:LB][0:LC][0:LD][0:N]) \
  ...
  runtime += tv2.tv_usec - tv1.tv_usec;
  fprintf(fp, "DATA,,0,0,%ld,%d,%d,%d,%d,%d,%d,%d,%d,%lld,%0.4f\n", device, VAR_STR[var].c_str(), mem_from, COUNT, LA, LB, LC, LD, LE, M, N, runtime, (double)(runtime/1000000.0));
  fflush(fp);
  printf("DATA,,0,0,%ld,%d,%d,%d,%d,%d,%d,%d,%d,%lld,%0.4f\n", device, VAR_STR[var].c_str(), mem_from, COUNT, LA, LB, LC, LD, LE, M, N, runtime, (double)(runtime/1000000.0));
