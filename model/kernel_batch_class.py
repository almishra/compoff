...
dr_columns = ['kernel', 'runtime'] 


dataset_root=""
single = pd.read_csv(dataset_root+"single.csv")
...
single["Class"].replace({"A":0}, inplace=True)
single["Class"].replace({"B":1}, inplace=True)
single["Class"].replace({"C":2}, inplace=True)

#single.astype({'Class': 'int64'}).dtypes
...
#single.astype({'Class': 'int32'}).dtypes
...
mod = KernelRunModel(ip_features=50, num_hidden=132, op_features=3).to(device)
...
#opt = torch.optim.Adamax(mod.parameters(), lr=1e-2,  weight_decay=1e-4)
...
for e in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        for index, (xt, yt) in enumerate(val_loader):
            ...
            total += _yt.size(0)
            correct += (predicted == _yt).sum().item()
        ...
        print(f'Accuracy of the network on val set: {100 * correct / total} %')
...
with torch.no_grad():
    ...
    for index, (xt, yt) in enumerate(te_loader):
        ...
        print(predicted.item(),',', _yt.cpu().data.numpy()[0])
        total_loss += loss1
